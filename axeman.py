...
async def processing_coro(download_results_queue, output_dir="/tmp"):
    ...
    while True:
        ...
        for entry in entries_iter:
            csv_storage = '{}/certificates/{}'.format(output_dir, entry['log_info']['url'].replace('https://', '').replace('/', '_'))
...
def process_worker(result_info):
    ...
    try:
        ...
        leaf_csv_file = "{}/leaf_{}-{}.csv".format(csv_storage, result_info['start'], result_info['end'])
        extra_data_csv_file = "{}/extra_{}-{}.csv".format(csv_storage, result_info['start'], result_info['end'])
        precert_leaf_csv_file = "{}/precert_leaf_{}-{}.csv".format(csv_storage, result_info['start'], result_info['end'])
        precert_extra_data_csv_file = "{}/precert_extra_{}-{}.csv".format(csv_storage, result_info['start'], result_info['end'])

        leaf_lines = []
        extra_data_lines = []
        precert_leaf_lines = []
        precert_extra_data_lines = []
        ...
        for entry in result_info['entries']:
            ...
            if mtl.LogEntryType == "X509LogEntryType":
                ...
                for c in chain:
                    try :
                        e = c.get_pubkey().to_cryptography_key().public_numbers().e
                        modn = c.get_pubkey().to_cryptography_key().public_numbers().n
                        leaf_lines.append("{:X},{},{},{}\n".format(modn, e, result_info['log_info']['url'], str(entry['cert_index'])))
                    except AttributeError as e:
                        continue
                  
                for cert in extra_data.Chain:
                    #chain.append(crypto.load_certificate(crypto.FILETYPE_ASN1, cert.CertData))
                    c = crypto.load_certificate(crypto.FILETYPE_ASN1, cert.CertData)
                    try :
                        e = c.get_pubkey().to_cryptography_key().public_numbers().e
                        modn = c.get_pubkey().to_cryptography_key().public_numbers().n
                        extra_data_lines.append("{:X},{},{},{}\n".format(modn, e, result_info['log_info']['url'], str(entry['cert_index'])))
                    except AttributeError as e:
                        continue
                ...
                for cert in extra_data.Chain:
                    ...
                    try :
                        e = c.get_pubkey().to_cryptography_key().public_numbers().e
                        modn = c.get_pubkey().to_cryptography_key().public_numbers().n
                        precert_extra_data_lines.append("{:X},{},{},{}\n".format(modn, e, result_info['log_info']['url'], str(entry['cert_index'])))
                    except AttributeError as e:
                        continue
        ...
        if (len(leaf_lines) != 0):
            with open(leaf_csv_file, 'w', encoding='utf8') as f:
                f.write("".join(leaf_lines))
        if (len(extra_data_lines) != 0):
            with open(extra_data_csv_file, 'w', encoding='utf8') as f:
                f.write("".join(extra_data_lines))
        if (len(precert_leaf_lines) != 0):
            with open(precert_leaf_csv_file, 'w', encoding='utf8') as f:
                f.write("".join(precert_leaf_lines))
        if (len(precert_extra_data_lines) != 0):
            with open(precert_extra_data_csv_file, 'w', encoding='utf8') as f:
                f.write("".join(precert_extra_data_lines))
        print("[{}] CSV {}, {}, {} and {} written!".format(os.getpid(), leaf_csv_file, extra_data_csv_file, precert_leaf_csv_file, precert_extra_data_csv_file))
